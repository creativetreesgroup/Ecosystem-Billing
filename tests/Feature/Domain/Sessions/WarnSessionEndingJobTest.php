...
test('a job with a matching token broadcasts the warning', function () {
    ...
    (new WarnSessionEndingJob($session->id, $session->expiry_token))->handle(app(DeviceManager::class));
...
test('a stale token does not broadcast a warning', function () {
    ...
    (new WarnSessionEndingJob($session->id, $staleToken))->handle(app(DeviceManager::class));
