...
class UnitForm
{
    public static function configure(Schema $schema): Schema
    {
        return $schema
            ->components([
                // Tabs, bukan seksi bertumpuk: tiga kelompok ini dipakai pada
                // WAKTU yang berbeda — identitas saat unit dibuat, kontrol TV
                // saat perangkatnya dipasang, catatan sesekali. Menumpuknya
                // memaksa operator menggulir melewati bagian yang sedang tidak
                // ia kerjakan setiap kali.
                Tabs::make()
                    ->columnSpanFull()
                    ->tabs([
                        Tab::make('Identitas unit')
                        ...
                        Tab::make('Kontrol TV')
                            ...
                            // Lencana memberi tahu SEBELUM tab dibuka: unit yang masih
                            // Manual berarti TV-nya belum terpasang sama sekali.
                            ->badge(fn (?Unit $record) => self::driverOf($record?->control_driver) === ControlDriver::Manual ? 'Manual' : null)
                            ->badgeColor('warning')
                            // ['md' => 2], bukan columns(2) — columns(2) di Filament
                            // berarti ['lg' => 2] dan tablet ikut menumpuk seperti HP.
                            ->columns(['md' => 2])
                        ...
                        Tab::make('Catatan')
                            ...
                            ->badge(fn (?Unit $record) => filled($record?->notes) ? '1' : null)
                            ->schema([
                                Textarea::make('notes')
                                    ->hiddenLabel()
